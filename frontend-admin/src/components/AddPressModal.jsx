...
export default function AddPressModal({ isOpen, onClose, onAdd }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setHeadline(''); setRedirectLink(''); setImage(null); setImagePreview('');
      onClose();
    } catch (error) {
      console.error(error);
    } finally {
      ...
      <div className="w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-800">
          <h2 className="text-lg font-semibold flex items-center gap-2">
            <ExternalLink className="w-4 h-4 text-neutral-400" /> Add Press Feature
          </h2>
        ...
        <form id="add-press-form" onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
          ...
          <div>
            ...
            <input type="url" value={redirectLink} onChange={(e)=>setRedirectLink(e.target.value)} required placeholder="https://"
              className="w-full bg-neutral-950 border border-neutral-800 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-neutral-500"
            />
          </div>
          <div>
            ...
            {imagePreview ? (
              <div className="relative w-full h-32 rounded-lg border border-neutral-800 overflow-hidden group">
                ...
                <button type="button" onClick={() => {setImage(null); setImagePreview('');}} className="absolute top-2 right-2 bg-black/60 text-white rounded p-1 hover:bg-red-500"><X className="w-4 h-4" /></button>
              </div>
            ) : (
              <div className="relative flex justify-center px-6 py-6 border border-neutral-800 border-dashed rounded-lg hover:border-neutral-600 bg-neutral-950">
                ...
                <div className="flex flex-col items-center text-sm text-neutral-400"><UploadCloud className="w-6 h-6 mb-2" /><span>Upload Image</span></div>
              </div>
            )}
          </div>
        </form>
        ...
        <div className="px-6 py-4 border-t border-neutral-800 flex justify-end gap-3 bg-neutral-950">
          ...
          <button type="submit" form="add-press-form" disabled={isSubmitting || !image} className="bg-white text-black px-6 py-2 rounded-lg text-sm font-semibold hover:bg-neutral-200 disabled:opacity-50">
